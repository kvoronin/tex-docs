...
As it will be clear from the transport case, higher-order elements might provide a much better results, even for the solutions which are less regular. To check this statement for the Laplace problem, in Table \ref{tab:ur_lapl_lshape3D_HdivH1lapl_feorder1} we provide results for higher-order elements, cf. with Table \ref{tab:ur_lapl_lshape3D_HdivH1lapl}.
...
In Table \ref{tab:amr2_conv_3D_HdivL2tran_feorder1} we show the results of adaptive mesh refinement for higher-order elements.

The higher-order elements (see Tables \ref{tab:ur_conv_3D_HdivL2tran_feorder1} and \ref{tab:amr2_conv_3D_HdivL2tran_feorder1},  and also Figure \ref{fig:amr_transp_feorder1}) provide much higher accuracy.
...
For Table \ref{tab:amr2_conv_3D_HdivL2tran_feorder1} one can find in Figure \ref{fig:amr_transp_feorder1} numerical solution snapshots for certain iterations.
...
\begin{itemize}
	...
	The mesh was constructed as a time cylinder with usual 3D Fichera corner in the base using the developed 4d mesh generator \cite{meshgen_text}.
	...
	$$
	\mbox{Gaussian hill 3D}(x,y,z,t) = exp^{-100 \left( (x-0.5)^2 + y^2 + (z-0.25)^2 \right) }
	$$
...
First, in Table \ref{tab:ur_lapl_fichera4D_HdivH1lapl_smooth} one can find convergence results for the smooth solution ($u \in H^{2.6}$) of the Fichera corner problem in 4D.
...
In Tables \ref{tab:ur_trans_cyl4D} and \ref{tab:amr_trans_cyl4D} we present results for uniform and adaptive mesh refinement for the rotating Gaussian sphere in 4D.
