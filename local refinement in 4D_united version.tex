...
\begin{abstract}
This paper is a yet unpublished draft of some (negative) results related to adaptive mesh refinement for four-dimensional simplicial meshes. The main idea was to extend results obtained by the cited paper by D. Arnold et al. In that paper its authors developed a local refinement procedure in 3D for tetrahedral meshes, proved theoretically that the number of similarity classes will be bounded independently of the number of refinement levels.
Since that algorithm can be considered as coming from Maubach's algorithm for n-simplices, it was natural to try to extend results in 4D case which is quite important for space-time approaches.

However, a negative result was obtained. That is, the extension ca be formulated but encounters some essential problems which prevents from getting the similar theoretical results. Another critical remark is that there is actually no bound (even in 3D) for how far the refinement closure will extend beyond the target group of simplices which are to be refined.

\end{abstract}
\maketitle

\section{Purpose of this document}
In the document a lot of details are given for certain adaptive refinement strategies in 4D. If anyone find this useful or interesting, one is free to contact the author for further discussion. The author can be wrong in some of his statements, and will be very grateful for pointing them out.
...
In three-dimensional case D. Arnold and his cauthors developed a similar algorithm which is tightly connected to Maubach's algorithm but is formulated slightly different.
...
As one can notice, when the initial tetrahedron has class $P_f$ in Arnold's algorithm , more new vertices than only midpoints of the edges are needed to be introduced if one decides to refine it with three level bisection. 
MFEM mesh structures feel inconvenient to do this and the modification idea is to simply don't let tetrahedrons have class $P_u$ at the start of the refinement process. To do this one can simply switch off the flag of the tetrahedron of class $P$ at the end of the previous refinement (or not set it during the initial tetrahedron marking process).
...
Unfortunately, this trick doesn't work directly in 4D, because now one cannot start with both $P_u$ and $P_f$ classes of pentatops. It is fine to unflag pentatops but changing their type requires special care because one should keep refinement edges in the neighbouring simplices compatible.
...
The idea is to use Luby's algorithm developed for finding maximal independent sets in graphs. 

Here we'd like to to assign weights to all the edges equal to the number of edges touching its two endpoints which belong to the target group of simplices.
Then we choose edges with locally maximal weights (using Luby's parallel algoritm) they will be inside the targeted group of course, if there are more than one simplex in the group) and bisect them. together with simplices which contain it (so we don't introduce any new vertices except the new midpoint).
...
But certainly there will be no theoretical bound on the number of similarity classes for such an algorithm.
...
\begin{itemize}
	...
	\item It looks like a trade-off in theory - either you do something like Arnold's algorithm and have a bound on the number of similarity classes or you use an algorithm which refines only a bounded number of neighboring layers (around the group of elements targeted for local refinement).
