...
It is implicitly assumed that the reader already knows the basic concepts of local refinement, e.g. what is a mesh closure, similarity classes, etc. 
...
Then, in section 4 we present Maubach's general algorithm from \cite{maubach} and apply it in 4D case. The resulting tree diagrams are presented in full details. Then, in section 5 we ...

In the appendix one can find tree diagrams for Maubach's algorithm applied in 3D case.
...
Notice, that we had to introduce additional points which are not edge midpoints:
$$
\begin{array}{cc}
\mbox{edge} & \mbox{midpoint} \\
\left[4,5\right] & 15 \\
...
Looking at the tree diagrams (Cases 1 - 4) we can extract the following rules for pentatop type transformations (just be checking the refinement edges for pentatops and their direct children, level after level):
...
Remark. As it is pointed out in \cite{arnold}, in his paper \cite{maubach} Maubach essentially considered only case 1. But since we aim at considering arbitrary unstructured meshes, we need all the cases. A similar problem as in cases 3 and 4 happens already in 3D. This situation might happen if we start our refinement process with a tetrahedron of type $P_f$, and the following illustration shows what happens in this case.

ILLUSTRATION


The rules above suggest the following type transformaton diagram (cf. with diagram in section 6) where subscripts $u$, $f$ for class A2, $r$ and $b$ for class P are introduced in order to distinguish two different ways of these class transformation which appeared in the rules above):
...
One could notice now that the rules which govern the type transformation do not uniquely define a specific algorithm since the same rule can lead to different choice of refinement edges for the children. (But what we need are geometrical recipes of bisecting any possible pentatop!)
...
However, there are still things left to be defined. In particular, as mentioned earlier, there are more pentatop types than only $P$'s and $A2$'s.

It is not fully understood how the specific rules for the rest of the types affect the problem of closure finiteness but still we must have a look at them.
Looking at the theoretical proof for Arnold's algorithm, we conclude that whenever additional pentatop types disappear after the first refinement of the initial mesh, the only thing we should care about is how to transform them into $P$ and $A2$ types.
